...
1. Make sure the server is running and that your simulator is open
  1. The server is by default running on http://localhost:3001, but if you decide to change this then you must also tell the client what the correct API uri is inside the `client/config/keys.js` file
2. Run `expo start`
